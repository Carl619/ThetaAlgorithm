...
// Called every frame
void UAIComponet::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
TArray<ACubeMap*>  UAIComponet::Search(int start, int goal)
{
...
bool  UAIComponet::LineOfSigth(ACubeMap* origin, ACubeMap* objective)
{
    // FHitResult will hold all data returned by our line collision query
    FHitResult Hit;

    // We set up a line trace from our current location to a point 1000cm ahead of us
    FVector TraceStart = origin->GetActorLocation() + FVector(0, 0, 30);
    FVector TraceEnd = objective->GetActorLocation() + FVector(0, 0, 30);
...
TArray<ACubeMap*> UAIComponet::Reconstruct(int current, int start, ACubeMap* currentNode, TMap<int, ACubeMap*> cameFrom)
{
...
ACubeMap* UAIComponet::LowestFvalue(TMap<int, ACubeMap*> nodes)
{
    float fValue = -1;
...
int  UAIComponet::NearestNodeID(FVector location) {
...
TArray<ACubeMap*>  UAIComponet::ReverseList(TArray<ACubeMap*> nodes)
{
    Algo::Reverse(nodes);
    return nodes;
}
void  UAIComponet::Restart() {
    for (int i = 0; i < Nodes.Num(); i++)
    {
...
void  UAIComponet::CalculateH(int goal) {
    for (int i = 0; i < Nodes.Num(); i++)
    {
        if (i != goal)
        {
